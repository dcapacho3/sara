...
Built on ROS 2 Jazzy and the new Gazebo (Harmonic), split into proper ROS 2 packages, and where new capabilities keep getting added.
...
- **Sim/real parity, different maps.** The same Nav2 stack, safety layer, and control code run unmodified against both a Gazebo model and the physical robot. Each environment is mapped independently with Cartographer (simulation uses `sara_navigation/maps/supermarket_map.yaml`, the real lab replica uses `sara_navigation/maps/labrobfinal_mask.yaml`), but both share the same 7x3 m layout: three shelf units and a checkout zone.
...
- **Priority based velocity arbitration.** `twist_mux` (`sara_safety/config/mux.yaml`) arbitrates every velocity source by priority: the weight cutoff at 255, lidar obstacle avoidance at 200, keyboard or gamepad teleop at 100, and Nav2 autonomous navigation at 50. The emergency topics always win.
...
| Layer | Components |
|---|---|
| **Model** | SQLite product database (`sara_apps/database/products.db`), product/location records |
...
This repo holds several ROS 2 packages side by side, not one monolithic package:

```
sara_description/   URDF/xacro robot description, meshes
sara_gazebo/         Gazebo world (Supermarket) and scenery models
sara_navigation/     Nav2 + Cartographer params, maps, EKF config, mapping/localization launch files
sara_safety/         twist_mux config, obstacle avoidance, speed limiting, teleop override, weight-cutoff lock
sara_perception/     Sensing nodes (weight sensor, future customer-following camera) — scaffolded, no nodes yet
sara_bringup/        Top-level entry points that tie the above together (sim, real robot)
sara_apps/           Customer/worker touchscreen apps, product database, route planning
...
```bash
ros2 run sara_apps saragui.py     # customer / worker select and navigate flow
ros2 run sara_apps workergui.py   # catalog manager
```

When you select destinations and start the flow, `saragui.py` (through `base_navgui.py`) launches `sara_safety`'s `mux.launch.py` and, depending on the Real/Simulation mode selected on the welcome screen, either `sara_bringup`'s `real_nav.launch.py` or `navagv.launch.py`, as background processes. The full `twist_mux` plus Nav2 stack comes up automatically on first use instead of needing to be started by hand.
...
```bash
ros2 run sara_safety key_teleop.py
```

**Building a new map** (only needed if you change the physical or simulated layout):

```bash
ros2 launch sara_navigation cartographerturtle.launch.py   # simulation
ros2 launch sara_navigation real_cartographer.launch.py    # real robot

# once satisfied with the mapped area:
ros2 run nav2_map_server map_saver_cli -f ~/ros_ws/src/sara/sara_navigation/maps/<name>
```

The saved map's filename then needs to be pointed to from `sara_bringup`'s `navagv.launch.py` (simulation) or `real_nav.launch.py` (real robot), and from the `load_map()` path in `sara_apps/scripts/base_navgui.py`.
...
Predecessor: [`dcapacho3/turtlemart`](https://github.com/dcapacho3/turtlemart) (frozen, thesis reference implementation).
