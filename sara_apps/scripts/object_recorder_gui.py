...
def get_source_db_path(package_name, db_filename):
    ...
    db_path = os.path.join(workspace_root, 'src', 'sara', package_name, 'database', db_filename)
    return db_path
...
class SecondWindow(ctk.CTkToplevel):
    ...
    def setup_logo(self, parent_frame):
        ...
        try:
            pkg_dir = get_package_share_directory('sara_apps')
    ...
    def launch_ros2_files(self):
        ...
        if self.mode == "Real":
            nav_cmd = "ros2 launch sara_bringup real_nav.launch.py map_yaml:=labrobfinal_mask.yaml"
        else:
            nav_cmd = "ros2 launch sara_bringup navagv.launch.py map_yaml:=supermarket_map.yaml"
    ...
    def launch_mux(self):
        ...
        cmd = "ros2 launch sara_safety mux.launch.py"
    ...
    def launch_basic_control(self):
        ...
        cmd = "ros2 launch sara_bringup basic_control.launch.py"
        process = subprocess.Popen(
            cmd,
            shell=True,
            stdout=subprocess.PIPE,
    ...
    def register_product(self):
        ...
        try:
            db_dir = get_source_db_path('sara_apps', 'products.db')
    ...
    def load_map(self):
        ...
        bringup_dir = get_package_share_directory('sara_navigation')
        if self.mode == "Real":
