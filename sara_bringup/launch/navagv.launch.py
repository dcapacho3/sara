...
def generate_launch_description():
    bringup_dir = get_package_share_directory('sara_bringup')
    description_dir = get_package_share_directory('sara_description')
    gazebo_dir = get_package_share_directory('sara_gazebo')
    navigation_dir = get_package_share_directory('sara_navigation')
    launch_dir = os.path.join(navigation_dir, 'launch')
    ...
    ekf_config_path = os.path.join(navigation_dir, 'config', 'ekf.yaml')
    ...
    declare_map_yaml_cmd = DeclareLaunchArgument(
        'map',
        default_value=os.path.join(navigation_dir, 'maps', 'supermarket_map.yaml'),
    ...
    declare_params_file_cmd = DeclareLaunchArgument(
        'params_file',
        default_value=os.path.join(navigation_dir, 'params', 'nav2_params_sim.yaml'),
    ...
    initial_pose_cmd = TimerAction(
        period=10.0,
        actions=[Node(package='sara_navigation', executable='initial_pose_pub.py',
                       parameters=[{'use_sim_time': use_sim_time}])])
