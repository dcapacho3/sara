...
def generate_launch_description():
  ...
  pkg_share = FindPackageShare(package='sara_bringup').find('sara_bringup')
  description_share = FindPackageShare(package='sara_description').find('sara_description')
  navigation_share = FindPackageShare(package='sara_navigation').find('sara_navigation')
  default_model_path = os.path.join(description_share, 'models/sara.urdf.xacro')

  default_rviz_config_path = os.path.join(pkg_share, 'rviz/nav2_config_v2.rviz')
  nav2_dir = FindPackageShare(package='nav2_bringup').find('nav2_bringup')
  nav2_launch_dir = os.path.join(nav2_dir, 'launch')
  static_map_path = os.path.join(navigation_share, 'maps', 'labrobfinal_mask.yaml')
  nav2_params_path = os.path.join(navigation_share, 'params', 'nav2_params_super.yaml')
  nav2_bt_path = FindPackageShare(package='nav2_bt_navigator').find('nav2_bt_navigator')
  behavior_tree_xml_path = os.path.join(nav2_bt_path, 'behavior_trees', 'navigate_w_replanning_and_recovery.xml')

  # Configuración para el filtro de Kalman Extendido (EKF)
  ekf_config_path = os.path.join(navigation_share, 'config', 'ekf.yaml')
  ...
  costmap_cleaner_node = Node(package='sara_navigation', executable='costmap_cleaner.py')

  # Nodo bloqueador de multiplexor por medio de balanza integrada
  locker_node = Node(package='sara_safety', executable='mux_locker.py')
